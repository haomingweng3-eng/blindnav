...
当前视频默认还启用 `--min-approach-area 0.01` 小目标确认门：远处小框即使出现短时面积增长，也暂不播报，需达到更大的相对面积才进入风险反馈。该门在现有安全距离基线上把 81 次可播放告警降到 21 次；再经过方向级普通告警预算后为 20 次，约 11.22 次/分钟，仍未达到可用误报目标。正向危险事件到位后必须重新校准，不能把它当成最终阈值。

多目标反馈还使用方向级告警预算：同一方向短窗口内的普通告警合并，避免汽车、行人和两轮车分别刷屏；同一帧的不同目标仍保留，危险级告警不受该预算抑制并可抢占普通提示。这是反馈层限流，不等于检测准确率提升。
