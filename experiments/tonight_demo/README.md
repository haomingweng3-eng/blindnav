...
后续加入 `min_approach_area=0.01` 小目标确认门后，同一批原始记录为 34 次原始告警、21 次可播放告警（11.7721 次/分钟），汽车告警从 50 次降为 2 次；再启用方向级普通告警预算后为 20 次（11.2150 次/分钟）。该结果是误报/刷屏抑制实验，不是准确率证明，也不是最终阈值；危险级告警仍可抢占普通提示。
