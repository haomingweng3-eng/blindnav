...
当前交接结论：仓库已有 13 段安全距离视频，适合作为 `safe_baseline_test`，不应直接拿来宣称模型准确率。队友当前优先级不是继续扩大模型类别，而是补采受控 near-miss、看起来很近但安全的反例、多目标场景，并把风险事件标注和检测框标注分开完成。具体拍摄与提交规范见 `DATASET_HANDOFF.md`。
