...
class EvaluateLabeledVideosTests(unittest.TestCase):
    def test_reports_video_level_recall_and_safe_false_positive_rate(self):
        ...
        self.assertEqual(metrics["conflict_video_count"], 0)
        self.assertEqual(metrics["conflict_recall"], 0.0)
        self.assertEqual(metrics["near_miss_video_count"], 1)
        self.assertEqual(metrics["near_miss_detection_rate"], 1.0)
