...
5. 再把 `FrameDetections` 接到现有风险引擎字段，最后接震动、提示音和 TTS。
...
- 尚未完成 Android 端 ByteTrack、风险仲裁、震动/提示音/TTS 真机闭环。
...
如果这一步稳定，再实现 `FrameDetections → 风险 JSON → FeedbackDispatcher`。不要先做地图、双摄、SLAM 或云端多模态功能。
